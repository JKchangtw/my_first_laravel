...
@section('main')
    <section class="form">
        <div class="container-fluid form">
            ...
            <div class="row">
                <h1 style="color:blue;font-weight:bold">編輯會員</h1>
            </div>
            <div class="row line mb-2"></div>
            {{-- enctype是為了讓我們可以傳圖片 --}}
            <form class="d-flex flex-column" action="/account/update/{{$user->id}}" method="post">
                @csrf


                {{-- <h4>目前圖片>>><img src="{{$banner->img_path}}" alt=""></h4> --}}
                <label for="banner_img">會員名稱更新</label>
                <input type="text" name="name" id="name" value="{{$user->name}}">

                <label for="">會員密碼更新</label>
                <input type="text" name="password" id="password" value="{{$user->password}}">

                <label for="">會員權限更新</label>
                <select name="power" id="power">
                    <option value="1" @if ($user->power==1) selected @endif>管理者</option>
                    <option value="2" @if ($user->power==2) selected @endif>一般使用者</option>
                </select>
                {{-- <input type="number" name="power" id="power" value="{{$user->power}}"> --}}
